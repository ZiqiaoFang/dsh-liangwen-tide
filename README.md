...
| 历史怎么补 | 启动时经 `ctx.sessionQuery` 列会话 + 读日志补一次（活着的会话直接读内存日志，零 IO）；**不按会话创建时间过滤**（长驻会话几天前创建、今天还在用，按创建时间过滤会让今日花费凭空少一大截），改由"事件时间视界"（最近 7 天）决定算不算 |
| 模型怎么认 | 事件流里的 `request/context`；插件是**会话中途才加载**时，这类事件在启动前就写进日志了，此时用会话自己的路由快照 `session.requestContext().model` 兜底 —— 否则这些调用会被记成"未匹配价格"（就是"计价模型获取失败"的表现） |
...
档位**真的翻转**的那一刻，从胶囊弹出一张抠像人像（进入谷时 → 图一 / 进入峰时 → 图二），
背后是旋转光芒，下面一条紧凑说明条带实时倒计时；停留 6 秒后自动收起。

| 项目 | 做法 |
| --- | --- |
| 触发 | 只在**真实换挡**且刚过点 60 秒内弹；页面刚打开不弹；标签页被挂起后恢复不补弹（避免突然蹦出几小时前的图） |
| 位置 | 挂载在根级悬浮层 `shell.overlay`（会话标题栏有 overflow 裁切，弹窗必须挂在它祖先之外），再用 `getBoundingClientRect()` 贴住胶囊 |
| 方向 | 胶囊在标题栏右端（上方是窗口顶边、右方是窗口右缘）→ **默认向下**弹；下方空间不够自动翻到上方；水平与胶囊**右对齐**并夹进视口留白。这段是纯函数 `placePopup()`，有单测 |
| 人像 | 抠像后的 webp（带 alpha、边缘羽化），由宿主半 `GET /liangwen-tide/asset/<valley\|peak>` 下发（白名单 + 只读 + 进程内缓存，不把 base64 塞进 bundle） |
...
| 当场演示 | 不用等真换挡：**Ctrl+Shift+Alt+T**（macOS 为 **⌘+Shift+Option+T**）连按即可在峰↔谷之间**交替**弹出（第 1 下弹"当前档位的相反档"，之后每按一下翻一面）；控制台 `__liangwenTide.preview('peak'\|'valley')`、`.hide()`、`.state()`（state 带锚点/定位诊断）。胶囊的悬停提示里也写了这个快捷键。**刻意避开了 `⌘/Ctrl+Shift+T`**——那是浏览器「重新打开刚关闭的标签页」的默认绑定，浏览器先吃到事件、页面拦不住，按一下会既弹窗又重开标签页 |
