...
export function createLedger(options = {}) {
  ...
  /** 只统计算得进来的时间窗（与保留天数一致），更老的事件直接略过。 */
  const horizonMs = keepDays * 24 * 60 * 60 * 1000

  /**
   * 吃一个事件；返回是否计入了一次调用。
   * @param sessionId - 会话 id
   * @param event - 会话事件
   * @param fallbackModel - 事件流里没出现过 request/context 时的兜底模型
   *   （例如插件是会话中途才加载的：那次调用的路由信息在插件启动之前就写进日志了）
   */
  function ingest(sessionId, event, fallbackModel) {
    ...
    // 早于保留窗口的事件：不计数（水位线已推进，后续事件不受影响）
    const eventTime = Number(event.time)
    if (Number.isFinite(eventTime) && Date.now() - eventTime > horizonMs) return false

    if (fallbackModel !== undefined && fallbackModel !== null && state.model === null) {
  ...
  /** 批量吃一个会话的日志（补历史用）。 */
  function ingestAll(sessionId, events, fallbackModel) {
    let counted = 0
    for (const event of events ?? []) if (ingest(sessionId, event, fallbackModel)) counted += 1
    return counted
  }
...
export async function backfillLedger(ledger, sources, options = {}) {
  ...
  for (const record of records) {
    ...
    if (id === '') continue
    try {
      const lastModelIn = (events) => {
        for (let index = (events?.length ?? 0) - 1; index >= 0; index -= 1) {
          const candidate = events[index]
          if (candidate?.type === 'request/context' && typeof candidate.data?.model === 'string') return candidate.data.model
        }
        return undefined
      }
      ...
      if (liveSession !== undefined) {
        ...
        result.calls += ledger.ingestAll(id, events, lastModelIn(events))
        result.sessions += 1
        continue
      }
      // 不按 header.createdAt 过滤：长驻会话（几天前创建、今天还在用）会被漏掉 ——
      // 那样今天的花费会凭空少一大截（踩过：12 个会话全被跳过，今日 ¥0.00）。
      // 真正的过滤交给"事件时间视界"（lib/usage.js 里按 keepDays 判断）。
      const snapshot = await sources.readSession(id)
      result.calls += ledger.ingestAll(id, snapshot?.events, lastModelIn(snapshot?.events))
      result.sessions += 1
    } catch (error) {
