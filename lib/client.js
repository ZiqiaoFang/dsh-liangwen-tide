...
.lwt-pop { position: absolute; display: flex; flex-direction: column; align-items: center;
  pointer-events: none; --lwt-glow: .75; }
...
.lwt-figure { position: relative; z-index: 1; height: var(--lwt-h); width: auto; display: block; opacity: 0;
  ...
  mask-image: linear-gradient(to bottom, #000 79%, rgba(0,0,0,.5) 92%, transparent 100%); pointer-events: none; }
.lwt-cap { position: relative; z-index: 3; margin-top: 4px; padding: 7px 11px 8px; border-radius: 11px;
  ...
  background: rgba(38,38,42,.82); box-shadow: 0 10px 26px rgba(0,0,0,.45); pointer-events: auto; }
html:not([data-dsh-theme="dark"]) .lwt-cap { background: rgba(255,255,255,.86); box-shadow: 0 8px 22px rgba(0,0,0,.16); }
...
`
     ...
     */
    function fireCelebration(phase, boundaryAt) {
      prefetchAssets()
      celebration = {
        phase,
    ...
    function TidePill() {
      ...
      if (anchorRef !== null && anchorRef.current !== null) pillNode = anchorRef.current
      const children = [
    ...
    function TideCelebration() {
      ...
      const layerRef = React.useRef === undefined ? null : React.useRef(null)
      ...
      useEffect(() => {
        ...
        const anchor = pillNode
        if (box !== null && anchor !== null && typeof anchor.getBoundingClientRect === 'function') {
          ...
          const placement = placePopup(anchor.getBoundingClientRect(), size, viewport)
          const origin = layer?.getBoundingClientRect?.() ?? { left: 0, top: 0 }
          active.dir = placement.dir
          active.left = placement.left - origin.left
          active.top = placement.top - origin.top
          active.placed = true
          box.style.left = `${active.left}px`
          box.style.top = `${active.top}px`
        }
      }, [active])

      if (active === null) return null
      const color = active.phase === 'peak' ? PEAK_COLOR : VALLEY_COLOR
      ...
      return h(
        ...
        {
          ref: layerRef,
          className: active.placed ? 'lwt-pop lwt-show' : 'lwt-pop',
          style: {
            ...
            left: `${active.left}px`,
            top: `${active.top}px`,
            visibility: active.placed ? 'visible' : 'hidden',
          },
          onMouseEnter: () => { active.hovering = true },
          onMouseLeave: () => { active.hovering = false },
        ...
        h('div', { className: 'lwt-cap' },
          ...
          h('div', { className: 'lwt-cap-sub' },
            `北京时间 ${beijingClock(active.boundaryAt)} 起 · ${tierLabelOf(active.phase)}${active.phase === 'valley' ? '半价' : '全价'}`),
