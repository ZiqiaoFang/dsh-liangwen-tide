...
.lwt-cap { position: relative; z-index: 3; margin-top: 4px; padding: 7px 11px 8px; border-radius: 11px;
  ...
  box-shadow: var(--dsw-elevation-prominent, 0 10px 26px rgba(0,0,0,.35)); pointer-events: auto; }
...
`
    ...
    function previewCelebration(requested) {
      const current = snapshot?.phase ?? phaseAt(Date.now())
      const phase = requested === 'peak' || requested === 'valley'
        ? requested
        : (current === 'peak' ? 'valley' : 'peak')
    ...
    function installPreviewHook() {
      ...
      window.addEventListener('keydown', (event) => {
        const combo = event.shiftKey && (event.metaKey || event.ctrlKey)
        if (!combo || String(event.key).toLowerCase() !== 't') return
